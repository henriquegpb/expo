...
internal class ExpoLauncherUrlTest {
  ...
  fun `app deep link carrying a reserved param`() {
    val launch = ExpoLauncherUrl(Uri.parse("myapp://login?__expo_launch_token=abc&x=1"))
  ...
  @Test
  fun `legacy host accepts disableFab and disableAutoLaunch`() {
    val launch = ExpoLauncherUrl(
  ...
  fun `empty launch token is null`() {
    val launch = ExpoLauncherUrl(Uri.parse("exp://h:8081?__expo_launch_token="))
